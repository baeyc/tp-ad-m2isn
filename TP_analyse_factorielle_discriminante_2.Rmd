...
Dans certains cas, et pour ceux qui veulent aller plus loin, le logo `r fa("fas fa-star", fill = "orange")` vous propose une autre solution à l'aide de packages complémentaires.
...
Diviser le jeu de données en une partie apprentissage, contenant la moitié des individus (choisis aléatoirement), et une partie test contenant les individus restants. 
 ...
 `r fa("fas fa-star", fill = "orange", height = 15)` *Utiliser le package `dplyr` et son outil `sample`*

```{r apprtest, exercise=TRUE}
...
Affichier la matrice de confusion. Quel est le taux de bien classés et de mal classés sur l'échantillon test ?

```{r afdpred, exercise=TRUE}
...
```
 ...
 `r fa("fas fa-star", fill = "orange", height = 15)` *Tracer le nuage de points des données d'apprentissage et les 2 axes discriminants, puis afficher les données de test et leurs affectations.*

```{r afdplot, exercise=TRUE}
...
```{r insectplot, exercise=TRUE, exercice.setup="insectafd-solution"}
...
```
 ...
 `r fa("fas fa-star", fill = "orange", height = 15)` *Afficher quelques exemples de chiffres mal classés. Que remarque t-on ?*
 
```{r exmisscl, exercise=TRUE}
